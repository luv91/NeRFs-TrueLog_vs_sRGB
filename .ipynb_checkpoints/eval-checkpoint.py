...
def main(unused_argv):
    ...
    if not config.eval_only_once:
        summary_writer = tensorboardX.SummaryWriter(os.path.join(config.exp_path, 'eval'))
    ...
    while True:
        ...
        for idx, batch in enumerate(dataloader):
            ...
            # Only compute metrics if we are not rendering a path
            if not config.render_path:
                rgb = postprocess_fn(rendering['rgb'])
                rgb_cc = postprocess_fn(rendering['rgb_cc'])
                rgb_gt = postprocess_fn(gt_rgb)
                utils.save_img_u8(rgb_gt, path_fn(f'groundtruth_{idx:03d}.png'))


                # <<< CHANGED >>> Compute a single-channel MSE map (avg across RGB)
                # shape: [H, W]
                diff_sq = (rgb - rgb_gt) ** 2  # shape [H, W, 3]
                mse_map = diff_sq.mean(axis=-1)  # single-channel [H, W]

                # Save MSE map as a 32-bit float image
                mse_map_path = path_fn(f'mse_map_{idx:03d}.tiff')
                utils.save_img_f32(mse_map, mse_map_path)
                logger.info(f'Saved MSE map to {mse_map_path}')

                overall_mse = mse_map.mean()
                logger.info(f'Overall MSE for image {idx}: {overall_mse:.6f}')

                # Evaluate standard metrics
                if config.eval_quantize_metrics:
                    # Ensures the images written to disk reproduce the same metrics
                    rgb = np.round(rgb * 255) / 255
                    rgb_cc = np.round(rgb_cc * 255) / 255

                if config.eval_crop_borders > 0:
                    cfn = lambda x, c=config.eval_crop_borders: x[c:-c, c:-c]
                    rgb = cfn(rgb)
                    rgb_cc = cfn(rgb_cc)
                    rgb_gt = cfn(rgb_gt)
                ...
                if config.compute_normal_metrics:
                    ...
                    for k, val in rendering.items():
                        if k.startswith('normals') and val is not None:
                            normalized_normals = ref_utils.l2_normalize_np(val)
                            metric[k + '_mae'] = ref_utils.compute_weighted_mae_np(
            ...
            # If saving output images at intervals
            if config.eval_save_output and (config.eval_render_interval > 0):
                if (idx % config.eval_render_interval) == 0:
                    ...
                    for key in ['distance_mean', 'distance_median']:
                        if key in rendering:
                            utils.save_img_f32(rendering[key], path_fn(f'{key}_{idx:03d}.tiff'))

                    if 'depth_triplet' in rendering:
                        utils.save_img_f32(rendering['depth_triplet'], path_fn(f'depth_triplet_{idx:03d}.tiff'))

                    for key in ['normals']:
                        if key in rendering:
                            utils.save_img_u8(rendering[key] / 2. + 0.5, path_fn(f'{key}_{idx:03d}.png'))
        ...
        if (not config.eval_only_once) and accelerator.is_main_process:
            ...
            for name in metrics_cc[0]:
                ...
                summary_writer.add_histogram('eval_metrics_cc/' + 'perimage_' + name, scores, step)
            ...
            for i, r, b in showcases:
                ...
                if not config.render_path:
                    ...
                    if config.compute_normal_metrics:
                        summary_writer.add_image(f'true_normals_{i}',
                                                 tb_process_fn(b['normals']) / 2. + 0.5,
                                                 step)

        # Write per-image metric logs
