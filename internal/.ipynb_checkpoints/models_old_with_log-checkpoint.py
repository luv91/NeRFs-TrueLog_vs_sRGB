...
class Model(nn.Module):
    ...
    num_nerf_samples: int = 16  # The number of samples the final nerf level.
...
class MLP(nn.Module):
    ...
    # net_depth_viewdirs: int = 2  # The depth of the second part of ML.
    # net_width_viewdirs: int = 64  # The width of the second part of MLP.
    ...
    grid_base_resolution: int = 2
    grid_disired_resolution: int = 8192
    grid_log2_hashmap_size: int = 21
    net_width_glo: int =  64 # The width of the second part of MLP.
    # net_width_glo: int = 64  # The width of the second part of MLP.
...
# ===========>
#             # Apply log transformation before sigmoid activation
            S2 = 255.0
            S1 = S2 / (torch.exp(torch.tensor(1.0)) - 1)

            # Convert from log space to linear RGB
            
            linear_rgb = ((torch.exp(self.rgb_layer(x)) - 1) * S1)
            clamped_linear_rgb = torch.clip(linear_rgb, min=1, max=S2)
            log_rgb = torch.log(clamped_linear_rgb)  # This is guaranteed to be within [0, log(S2)]

            # Apply SiLU activation on log_rgb
            transformed_rgb = torch.nn.SiLU()(log_rgb)
            
            reverse_linear_rgb = torch.exp(transformed_rgb)
            reverse_go_pro_mode_rgb = torch.log((reverse_linear_rgb / S1) + 1)  # Reverse the transformation applied earlier
            rgb = reverse_go_pro_mode_rgb
            # rgb_swish = rgb * torch.sigmoid(self.rgb_premultiplier * rgb + 
            #                     self.rgb_bias)
            # rgb = rgb_swish
            
            rgb = torch.sigmoid(self.rgb_premultiplier * rgb + 
                                self.rgb_bias)
